	...
	if (g_tunnels.find(conn->name()) == g_tunnels.end()) 
	{
		...
		else if (buf->readableBytes() > 8) 
		{
			...
			if (where != end) 
			{
				...
				if (ver == 4 && cmd == 1 && okay) 									// CONNECT请求
				{
					TunnelPtr tunnel(new Tunnel(g_eventLoop, serverAddr, conn));
					tunnel->setup();
					tunnel->connect();
					g_tunnels[conn->name()] = tunnel;								// 每个Client对应一个tunnel
					buf->retrieveUntil(where+1);
					char response[] = "\000\x5aUVWXYZ";								// 请求得到允许的回应包
				...
				{
					char response[] = "\000\x5bUVWXYZ";								// 请求被拒绝或失败的回应包
					conn->send(response, 8);
					conn->shutdown();
				}
			}
