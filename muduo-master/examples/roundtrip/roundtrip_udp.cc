...
void clientTimeOutCallback(int sockfd)
{
	LOG_INFO << "clientTimeOutCallback";
}

void clientErrorCallback(int sockfd)
{
...
void runClient(const char* ip, uint16_t port)
{
	...
	//设置套接字epoll超时回调
	channel.setTimeoutCallback(boost::bind(&clientTimeOutCallback, sock.fd()));
	//设置套接字错误回调，避免不处理导致出现epoll一直触发的问题
	channel.setErrorCallback(boost::bind(&clientErrorCallback, sock.fd()));
	channel.enableReading();
	//loop.runEvery(0.2, boost::bind(sendMyTime, sock.fd()));
	//0.2秒后调用sendMyTime(sock.fd())，也可以直接调用
	loop.runAfter(0.2, boost::bind(sendMyTime, sock.fd()));
	loop.loop();
}
