...
private:
	...
	TcpClient client_;				//客户端，只管理一个会话(TCP连接)		
	Client* owner_;
	int64_t bytesRead_;
...
public:
	...
	void onDisconnect(const TcpConnectionPtr& conn)
	{
		if (numConnected_.decrementAndGet() == 0)
		{
			...
			for (boost::ptr_vector<Session>::iterator it = sessions_.begin();it != sessions_.end(); ++it) 
			{
