...
std::pair<int, int> runOnce()
{
	...
	// 向另一个Unix域套接字写数据
	int space = (numPipes / numActive) * 2;
...
{
	numPipes = 100;			// 类似100个网络连接
	numActive = 1;			// 类似1朵花
	numWrites = 100;		// 类似花转手100次
	...
	// 创建numPipes个Channel(每个Channel绑定一个Unix域套接字)
	for (int i = 0; i < numPipes; ++i) 
	{
	...
	for (int i = 0; i < 25; ++i) 
	{
		...
		printf("iterTime:%8d loopTime:%8d\n", t.first, t.second);
	}

	for (boost::ptr_vector<Channel>::iterator it = g_channels.begin(); it != g_channels.end(); ++it)
	{
		it->disableAll();
		it->remove();
	}
